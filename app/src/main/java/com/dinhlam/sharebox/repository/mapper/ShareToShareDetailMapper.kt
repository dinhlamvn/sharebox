...
class ShareToShareDetailMapper @Inject constructor(
    ...
    fun map(share: Share): ShareDetail {
        val shareData =
            when (val shareType = enumByNameIgnoreCase(share.shareType, ShareType.UNKNOWN)) {
                ...
                ShareType.IMAGES -> gson.fromJson(
                    share.shareData, ShareData.ShareImages::class.java
                )

                else -> error("Has error when parse share type $shareType")
            }
        return ShareDetail(
            share.id, share.shareUserId, share.shareNote.orEmpty(), share.createdAt, shareData
        )
    }
}